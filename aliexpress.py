...
def get_total_page(url):
    ...
    try:
        ...
        tpage = int(soup.select('#simple-pager')
                    [0].select('.ui-label')[0].text.split('/')[1])
        total_page = tpage
...
class CommentSpyder(threading.Thread):
    ...
    def parse_comment(self, comment_html):
        ...
        try:
            ...
            for i in range(0, len(fb_mains)):
                temp = []
                try:
                    username = user_info[i].select(
                        '.user-name')[0].text.replace('\t', '').replace('\n', '')
                except Exception as e:
                    username = ''
                try:
                    usercountry = user_info[i].select(
                        '.user-country')[0].text.replace('\t', '').replace('\n', '')
                except:
                    usercountry = ''
                try:
                    star = int(fb_mains[i].select(
                        '.star-view')[0].select('span')[0]['style'].lstrip('width:').rstrip('%'))/20
                except Exception as e:
                    star = 0
                try:
                    buyer_review = fb_mains[i].select('.buyer-review')[0]
                    buyer_feedback = buyer_review.select('.buyer-feedback')[0].select(
                        'span')[0].text.lstrip(' ').rstrip(' ').replace('\t', ' ').replace('\n', ' ')
                except Exception as e:
                    buyer_feedback = ''
                try:
                    ...
                    feedback_time = ''
                try:
                    additional_review = fb_mains[i].select(
                        '.buyer-addition-feedback')
                    if len(additional_review) > 0:
                        additional_feedback = additional_review[0].text.lstrip(
    ...
    def crawl_comment_by_page(self, page):
        ...
        try:
            ...
            comment_html = self.session.post(
                self.posturl, headers=self.headers, data=self.data, timeout=30, proxies=proxies)
...
class Saver(threading.Thread):
    ...
    def save_data_to_db(self, data):
        try:
            ...
            cursor.execute('insert into comment (no,productid,username,usercountry,buyer_feedback,star,feedback_time,additional_feedback) values (%s,%s,%s,%s,%s,%s,%s,%s)', [
                           data['no'], self.productid, data['username'], data['usercountry'], data['buyer_feedback'], data['star'], data['feedback_time'], data['additional_feedback']])
            self.conn.commit()
        except Exception as e:
            print("插入数据出错")
            print(e)
...
def main(productid, owner_memberid, companyid, thread_num=10):
    ...
    if total_page > 0:
        ...
        for i in range(thread_num):
            ...
            if(i == (thread_num-1)):
                end_page = total_page
            spyder = CommentSpyder(
                url, productid, owner_memberid, companyid, result_queue, start_page, end_page)
